...
cd $CUSTOM_BARRIO
for file in *saa.*; do mv $file ${file//saa/$CUSTOM_BARRIO}; done
for file in config/*/*saa.*; do mv $file ${file//saa/$CUSTOM_BARRIO}; done
mv {_,}$CUSTOM_BARRIO.theme
grep -Rl saa .|xargs sed -i '' -e "s/saa/$CUSTOM_BARRIO/"
